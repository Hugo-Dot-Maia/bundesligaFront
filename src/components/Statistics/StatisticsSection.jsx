...
const StatisticsSection = () => {
  ...
  const calculateStatistics = (teams) => {
    ...
    const bestAttack = teams.reduce(
      (prev, current) => (prev.goalsFor > current.goalsFor ? prev : current),
      teams[0],
    ); // Provide initial value as the first team

    const bestDefense = teams.reduce(
      (prev, current) =>
        prev.goalsAgainst < current.goalsAgainst ? prev : current,
      teams[0],
    ); // Provide initial value as the first team

    const worstAttack = teams.reduce(
      (prev, current) => (prev.goalsFor < current.goalsFor ? prev : current),
      teams[0],
    ); // Provide initial value as the first team

    const worstDefense = teams.reduce(
      (prev, current) =>
        prev.goalsAgainst > current.goalsAgainst ? prev : current,
      teams[0],
    ); // Provide initial value as the first team
  ...
  const { bestAttack, bestDefense, worstAttack, worstDefense } =
    calculateStatistics(seasonResult);
